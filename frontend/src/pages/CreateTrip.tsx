  ...
  return (
    <div className="max-w-4xl mx-auto mt-4 mb-20 px-4">
      <div className="mb-10 mt-6 relative overflow-hidden px-4">
        <div className="absolute top-1/2 left-0 w-full h-1 bg-slate-200 dark:bg-[#263452] -translate-y-1/2"></div>
        <div 
          className="absolute top-1/2 left-0 h-1 bg-[#6366F1] -translate-y-1/2 transition-all duration-500" 
          ...
          {STEPS.map((s, i) => (
            <div key={s.id} className="flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs transition-colors duration-300 ${
                i < step ? 'bg-[#4F46E5] text-white' : i === step ? 'bg-[#6366F1] text-white border-2 border-[#818CF8]' : 'bg-[#E2E8F0] text-[#64748B] dark:bg-[#1E293B] dark:text-[#94A3B8]'
              }`}>
                {i < step ? <Check className="w-4 h-4" /> : i + 1}
              </div>
              <span className={`text-[10px] md:text-xs mt-1 hidden md:block ${i === step ? 'text-text-primary font-bold' : i < step ? 'text-text-secondary font-medium' : 'text-[#64748B] dark:text-[#94A3B8] font-medium'}`}>
                {s.title}
              </span>
            </div>
          ))}
      ...
      <Card className="min-h-[450px] flex flex-col p-6 md:p-10 shadow-lg border border-slate-200 dark:border-slate-800">
          ...
          >
            ...
            {step === 0 && (
              <div className="max-w-xl mx-auto space-y-6 text-center">
                <h2 className="text-3xl font-bold text-text-primary">Where are you starting from?</h2>
                <p className="text-text-secondary">Let us know your starting point to calculate transport costs.</p>
            ...
            {step === 1 && (
              <div className="max-w-xl mx-auto space-y-6 text-center">
                <h2 className="text-3xl font-bold text-text-primary">Where do you want to go?</h2>
                <p className="text-text-secondary">Pick your dream destination.</p>
                <div className="mt-8 text-left">
                  <Input 
                    label="Destination" 
            ...
            {step === 2 && (
              <div className="max-w-xl mx-auto space-y-6 text-center">
                <h2 className="text-3xl font-bold text-text-primary">When are you traveling?</h2>
            ...
            {step === 3 && (
              <div className="max-w-xl mx-auto space-y-6 text-center">
                <h2 className="text-3xl font-bold text-text-primary">Who's coming along?</h2>
                <div className="mt-8">
            ...
            {step === 4 && (
              <div className="max-w-xl mx-auto space-y-6 text-center">
                <h2 className="text-3xl font-bold text-text-primary">What's your total budget?</h2>
                <p className="text-text-secondary mb-8">Move the slider to set your total budget for the whole trip.</p>
            ...
            {step === 5 && (
              <div className="max-w-3xl mx-auto text-center">
                <h2 className="text-3xl font-bold mb-8 text-text-primary">Select your travel style</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {TRAVEL_STYLES.map(style => (
                    <div 
                      key={style.id} 
                      onClick={() => setFormData({...formData, travelStyle: style.id})}
                      className={`p-6 rounded-xl border-2 cursor-pointer transition-all ${formData.travelStyle === style.id ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20 text-text-primary' : 'border-slate-200 dark:border-border text-text-primary hover:border-primary-300 dark:hover:border-primary-600'}`}
                    >
            ...
            {step === 6 && (
              <div className="max-w-2xl mx-auto text-center">
                <h2 className="text-3xl font-bold mb-8 text-text-primary">What are your interests?</h2>
            ...
            {step === 7 && (
              <div className="max-w-2xl mx-auto text-center">
                <h2 className="text-3xl font-bold mb-8 text-text-primary">Preferred Transportation</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
                  {TRANSPORT_OPTIONS.map(opt => (
                    <label key={opt.id} className={`flex items-center p-4 border rounded-xl cursor-pointer ${formData.transportation === opt.id ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20 text-text-primary' : 'border-slate-200 dark:border-border text-text-primary'}`}>
            ...
            {step === 8 && (
              <div className="max-w-2xl mx-auto text-center">
                <h2 className="text-3xl font-bold mb-8 text-text-primary">Preferred Accommodation</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
                  {ACCOMMODATION_OPTIONS.map(opt => (
                    <label key={opt.id} className={`flex items-center p-4 border rounded-xl cursor-pointer ${formData.accommodation === opt.id ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20 text-text-primary' : 'border-slate-200 dark:border-border text-text-primary'}`}>
            ...
            {step === 9 && (
              <div className="max-w-2xl mx-auto text-center">
                <h2 className="text-3xl font-bold mb-8 text-text-primary">Food Preferences</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
                  {FOOD_PREFERENCES.map(opt => (
                    <label key={opt.id} className={`flex items-center p-4 border rounded-xl cursor-pointer ${formData.foodPreference === opt.id ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20 text-text-primary' : 'border-slate-200 dark:border-border text-text-primary'}`}>
            ...
            {step === 10 && (
              <div className="max-w-2xl mx-auto">
                <div className="text-center mb-8">
                  ...
                  <p className="text-text-secondary">Review your trip details before we generate the perfect itinerary.</p>
                </div>
                
                <div className="bg-slate-50 dark:bg-surface-hover rounded-2xl p-6 space-y-4 text-text-primary border border-transparent dark:border-border">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <div className="text-text-secondary mb-1">Route</div>
                      <div className="font-bold text-base">{formData.startingLocation} → {formData.destination}</div>
                    </div>
                    <div>
                      <div className="text-text-secondary mb-1">Dates</div>
                      <div className="font-bold text-base">{formData.startDate} to {formData.endDate}</div>
                    </div>
                    <div>
                      <div className="text-text-secondary mb-1">Travelers & Budget</div>
                      <div className="font-bold text-base">{formData.travelers} people • ₹{formData.budget.toLocaleString()}</div>
                    </div>
                    <div>
                      <div className="text-text-secondary mb-1">Style</div>
                      <div className="font-bold text-base capitalize">{formData.travelStyle}</div>
                    </div>
                  </div>
                  <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
                    <div className="text-text-secondary mb-2 text-sm">Interests</div>
        ...
        <div className="flex justify-between items-center mt-8 pt-6 border-t border-slate-200 dark:border-border">
